...
function copyStaticFiles() {
  return {
    ...
    closeBundle() {
      ...
      // Copy manifest.json
      copyFileSync(
        resolve(__dirname, 'src/safari/manifest.json'),
      ...
      const iconsDir = resolve(__dirname, 'src/safari/public/icons');
      ...
      // Create _locales for Safari
      const localesDir = resolve(__dirname, outDir, '_locales/en');
...
export default defineConfig({
  build: {
    ...
    rollupOptions: {
      input: {
        background: resolve(__dirname, 'src/safari/background.ts'),
        content: resolve(__dirname, 'src/safari/content.ts'),
