/**
 * Content script for FIL(L)M Chrome extension
...
async function handleSelectionComplete(selection: Selection): Promise<void> {
  ...
  // Extract DOM and styles immediately (before showing modal)
  const extractionOptions = {
    includeStyles: true,
    styleProperties: settings.fullStyles ? undefined : undefined, // Use filtered by default
  };

  const { dom, styles } = extractCapture(selection.elements, extractionOptions);

  // Calculate the selection rect with padding
  const viewport = getViewport();
  const paddedRect = padRect(selection.rect, 10);
  const clampedRect = clampToViewport(paddedRect, viewport);

  // Build partial capture for the modal
  const partialCapture: Partial<CaptureResult> = {
    ...
    dom,
    styles,
    screenshot: '', // Will be filled async
  };

  // Show the note modal immediately
 ...
 * Show the note modal
 */
function showNoteModal(partialCapture: Partial<CaptureResult>, rect: { x: number; y: number; width: number; height: number }): void {
  ...
  const estimatedSize = partialCapture.dom ? formatBytes(estimateSize(partialCapture as CaptureResult)) : undefined;
...
async function requestScreenshot(): Promise<string | null> {
  try {
    const response: ScreenshotResponse = await chrome.runtime.sendMessage({
